...
import com.example.mygallery.models.Album2;
...
public class AlbumUtility {
    ...
    public ArrayList<Album2> getAllAlbumData() {
        Gson gson = new Gson();
        Type type = new TypeToken<ArrayList<Album2>>(){}.getType();
    ...
    public void setAllAlbumData(ArrayList<Album2> data) {
        Gson gson = new Gson();
        SharedPreferences.Editor editor = sharedPreferences.edit();
    ...
    public void editAlbumName(String oldName, String newName) {
        ArrayList<Album2> data = getAllAlbumData();
        if (data != null) {
            for (Album2 d: data) {
    ...
    private void initAlbumData() {
        ArrayList<Album2> albumData = new ArrayList<Album2>();
        albumData.add(new Album2("Animals", new ArrayList<String>()));
        albumData.add(new Album2("Food", new ArrayList<String>()));
        albumData.add(new Album2("Holiday", new ArrayList<String>()));
        SharedPreferences.Editor editor = sharedPreferences.edit();
        Gson gson = new Gson();
    ...
    public boolean addNewAlbum(String albumName) {
        ...
        if (albums != null && data != null)
            if (albums.add(albumName) && data.add(new Album2(albumName, new ArrayList<String>()))) {
                setAllAlbums(albums);
                setAllAlbumData(data);
                return true;
            }
    ...
    public boolean addPictureToAlbum(String albumName, String picturePath) {
        ArrayList<Album2> data = getAllAlbumData();
        if (null != data) {
            Album2 selectedAlbum = findDataByAlbumName(albumName);
    ...
    public boolean deletePictureInAlbum(String albumName, String picturePath) {
        ...
        Album2 albumData = findDataByAlbumName(albumName);
        if (albumData != null) {
    ...
    public Album2 findDataByAlbumName(String albumName) {
        ArrayList<Album2> data = getAllAlbumData();
        if (null != data)
            for (Album2 d : data)
